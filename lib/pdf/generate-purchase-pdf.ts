...
export async function generatePurchaseInvoicePDF(data: InvoicePDFData) {
  const doc = new jsPDF()
  ...
  // Purchase Invoice Title - Centered
  doc.setFontSize(24)
  doc.setTextColor(59, 130, 246)
  doc.setFont("helvetica", "bold")
  doc.text("Purchase Invoice", pageWidth / 2, yPos + 20, { align: "center" })
  yPos += 35
  ...
  doc.text(`Paid: ${formatCurrency(received)}`, summaryX, currentY)
  currentY += 7
  ...
  // Generate filename
  const filename = `Purchase-${data.invoiceNumber}-${new Date(data.date).toISOString().split("T")[0]}.pdf`
