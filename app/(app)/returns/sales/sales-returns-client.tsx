...
export function SalesReturnsClient({ returns }: { returns: ReturnItem[] }) {
  ...
  const handleDelete = (returnNumber: string, id: string) => {
    if (!confirm(`Delete return ${returnNumber}? Stock will be unwound, refunds removed, and the original invoice status recomputed. This cannot be undone.`)) return
    setDeletingId(id)
                ...
                <tr key={r.id} className="hover:bg-muted/50">
                  ...
                  <td className="py-2 sm:py-3 px-2 sm:px-4 w-[10%]">
                    <div className="flex items-center gap-1">
                      ...
                      {/* RF-H1: delete is only available on Completed returns. The server
                          guards this too — UI just hides the affordance on others. */}
                      {r.status === "Completed" && (
                        <Button
                          variant="ghost"
                          size="icon"
