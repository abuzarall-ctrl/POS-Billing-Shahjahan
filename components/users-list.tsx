...
import { removeUser } from "@/app/(app)/users/actions"
...
export function UsersList({ initialUsers }: UsersListProps) {
  ...
  const confirmDelete = () => {
    ...
    startTransition(async () => {
      const result = await removeUser(deletingUserId)
      if (result.success) {
        ...
        <CardContent>
          {users.length === 0 ? (
            <div className="text-center py-8 text-muted-foreground">
              ...
              <p className="text-sm mt-2">Click "Create User" to add a new sub-user</p>
            </div>
          ) : (
